...
import { Search, Package, User, ShoppingCart, X, Wrench, ArrowRight, Hash } from 'lucide-react';
...
/* ── tipos de resultado ─────────────────────────────────────────────────── */
const TYPE = {
    product:  { icon: Package,      color: 'text-blue-600 bg-blue-50',    label: 'Producto' },
    customer: { icon: User,          color: 'text-emerald-600 bg-emerald-50', label: 'Cliente' },
    order:    { icon: Wrench,        color: 'text-violet-600 bg-violet-50', label: 'Taller' },
    sale:     { icon: ShoppingCart,  color: 'text-amber-600 bg-amber-50',  label: 'Venta' },
};
...
const GlobalSearch = () => {
    ...
    useEffect(() => {
        if (isOpen) { setTimeout(() => inputRef.current?.focus(), 50); setQuery(''); setResults([]); setSelected(0); }
    }, [isOpen]);
    ...
    const search = useCallback(async (q) => {
        ...
        try {
            ...
            custData.slice(0,3).forEach(c => out.push({
                ...
                title: c.name || c.full_name, subtitle: c.id_number || c.email || null,
                meta: null,
                action: () => navigate('/sales-center?tab=clientes'),
            }));
            ordData.slice(0,3).forEach(o => out.push({
                id: `o-${o.id}`, type: 'order',
                title: o.ticket_number || `Orden #${o.id}`, subtitle: o.device_description || null,
                meta: o.status,
    ...
    const handleKeyDown = (e) => {
        ...
        const list = results.length ? results : QUICK.map((q,i) => ({ id: `q-${i}` }));
        if (e.key === 'ArrowDown') { e.preventDefault(); setSelected(s => Math.min(s+1, list.length-1)); }
        if (e.key === 'ArrowUp')   { e.preventDefault(); setSelected(s => Math.max(s-1, 0)); }
        if (e.key === 'Enter')     {
            if (results.length > 0 && results[selected]) { results[selected].action(); setIsOpen(false); }
            else if (!query && QUICK[selected]) { navigate(QUICK[selected].path); setIsOpen(false); }
        }
    };
                                ...
                                <div className="py-1 divide-y divide-slate-50">
                                    {results.map((r, i) => (
                                        <div key={r.id} className={selected === i ? 'bg-slate-50' : ''}>
                                            <ResultRow {...r} onClick={() => { r.action(); close(); }} />
                                        </div>
                                    ))}
                                </div>
                            )}
                                ...
                                <div className="p-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-2">
                                        {QUICK.map((q, i) => {
                                            const Icon = q.icon;
                                            return (
                                                <button key={q.path} onClick={() => { navigate(q.path); close(); }}
