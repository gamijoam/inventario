                ...
                <header className="flex flex-col gap-3 border-b border-slate-200 bg-white px-5 py-4 lg:flex-row lg:items-center lg:justify-between">
                    <div className="flex min-w-0 items-center gap-3">
                        ...
                        <div className="min-w-0">
                            <p className="text-[11px] font-black uppercase tracking-wide text-slate-400">Auditoria de arqueo</p>
                            <h3 className="truncate text-xl font-black text-slate-900">Sesion #{session?.id}</h3>
                ...
                {loading ? (
                    ...
                    <div className="flex-1 overflow-y-auto px-5 py-4">
                        <section className="grid grid-cols-1 gap-3 md:grid-cols-2 xl:grid-cols-4">
                            ...
                            <MetricCard icon={Wallet} label="Esperado caja" value={money(summary.cash_expected_total_display_only)} tone="emerald" />
                            <MetricCard
                                icon={AlertTriangle}
                                label="Diferencia"
                                value={money(summary.cash_difference_total_display_only)}
                                tone={Math.abs(Number(summary.cash_difference_total_display_only || 0)) > 0.01 ? 'rose' : 'emerald'}
                            />
                        </section>
                            ...
                            <section className="mt-4 rounded-xl border border-amber-200 bg-amber-50 px-4 py-3">
                                ...
                                <div className="space-y-2">
                                    {alerts.map((alert, index) => (
                                        <div key={`${alert.code}-${index}`} className="rounded-lg bg-white/70 px-3 py-2 text-xs font-semibold text-amber-900">
                                            {alert.message}
                                        </div>
                                    ))}
                                </div>
                        ...
                        <section className="mt-4 grid grid-cols-1 gap-4 xl:grid-cols-[1.15fr_0.85fr]">
                            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
                                <div className="border-b border-slate-100 px-4 py-3">
                                    <h4 className="text-sm font-black text-slate-900">Caja por moneda</h4>
                                    <p className="text-xs font-semibold text-slate-500">Calculado desde saldo inicial, ventas, devoluciones, avances y movimientos.</p>
                                </div>
                                <div className="grid grid-cols-1 gap-3 p-4 lg:grid-cols-2">
                                    {cashRows.map((row) => (
                                        <div key={row.currency} className="rounded-xl border border-slate-200 bg-slate-50 p-3">
                                            <div className="mb-3 flex items-center justify-between">
                                                ...
                                                <span className={clsx(
                                                    ...
                                                    {Math.abs(Number(row.difference || 0)) <= 0.01 ? 'Cuadra' : 'Diferencia'}
                                                </span>
                                            </div>
                                            <MoneyLine label="Inicial" value={row.initial} currency={row.currency} money={money} />
                                            <MoneyLine label="Ventas efectivo" value={row.cash_sales} currency={row.currency} money={money} positive />
                                            ...
                                            <div className="mt-3 border-t border-dashed border-slate-300 pt-3">
                                                ...
                                                <MoneyLine label="Reportado" value={row.reported} currency={row.currency} money={money} strong />
                                                <MoneyLine label="Diferencia" value={row.difference} currency={row.currency} money={money} strong danger={Math.abs(Number(row.difference || 0)) > 0.01} />
                                            </div>
                                        </div>
                                    ))}
                                </div>
                            ...
                            <div className="rounded-xl border border-slate-200 bg-white shadow-sm">
                                <div className="border-b border-slate-100 px-4 py-3">
                                    ...
                                    <p className="text-xs font-semibold text-slate-500">Totales por metodo y moneda dentro de la sesion.</p>
                                </div>
                                <div className="max-h-[360px] space-y-2 overflow-y-auto p-4">
                                    {paymentMethods.length === 0 ? (
                                        ...
                                        <div key={`${method.method}-${method.currency}`} className="flex items-center justify-between gap-3 rounded-xl border border-slate-200 bg-slate-50 px-3 py-2">
                                            <div className="min-w-0">
                                                <p className="truncate text-sm font-black text-slate-800" title={method.method}>{method.method}</p>
                                                <p className="text-[11px] font-bold text-slate-400">{method.count} mov. / {method.currency}</p>
                                            </div>
