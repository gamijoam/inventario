...
const RuleModal = ({ rule, categories, onSave, onClose }) => {
    ...
    return (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-indigo-950/40 p-4">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-md">
                ...
                <div className="p-5 space-y-4">
                    <div>
                        ...
                        <input value={form.name} onChange={e => setForm(p => ({ ...p, name: e.target.value }))}
                            placeholder="Ej: Celulares 10%" className="w-full p-2.5 border rounded-md outline-none focus:ring-2 focus:ring-indigo-500 text-sm" />
                    </div>
                    <div>
                        <label className="block text-sm font-semibold text-slate-700 mb-1">Categoría *</label>
                        <select value={form.category_id} onChange={e => setForm(p => ({ ...p, category_id: e.target.value }))}
                            className="w-full p-2.5 border rounded-md outline-none focus:ring-2 focus:ring-indigo-500 text-sm bg-white">
                    ...
                    <div className="grid grid-cols-2 gap-3">
                        ...
                        <div>
                            ...
                            <input type="number" min="0" max="100" step="0.5" value={form.percentage}
                                ...
                                placeholder="10" className="w-full p-2.5 border rounded-md outline-none focus:ring-2 focus:ring-indigo-500 text-sm" />
                        </div>
                    </div>
                ...
                <div className="flex gap-3 p-5 border-t">
                    <button onClick={onClose} className="flex-1 py-2.5 border border-slate-200 rounded-md font-semibold text-slate-600 hover:bg-slate-50 transition-colors">Cancelar</button>
                    <button onClick={handleSave} disabled={saving}
                        className="flex-1 py-2.5 bg-indigo-600 text-white rounded-md font-bold hover:bg-indigo-700 transition-colors disabled:opacity-50">
        ...
        <div className="space-y-5 max-w-5xl">
            ...
            <div className="flex justify-end">
                <button onClick={() => setShowGuia(true)}
                    className="flex items-center gap-2 px-4 py-2 text-sm font-semibold text-indigo-600 bg-indigo-50 hover:bg-indigo-100 border border-indigo-200 rounded-md transition-colors">
                    <BookOpen size={16} /> Ver Guía
                </button>
            </div>

            <div className={`rounded-lg border p-5 ${settings?.global_enabled ? 'border-emerald-300 bg-emerald-50' : 'border-slate-200 bg-slate-50'}`}>
                <div className="flex items-center justify-between">
                    <div className="flex items-center gap-3">
                        <div className={`p-3 rounded-md ${settings?.global_enabled ? 'bg-emerald-100 text-emerald-700' : 'bg-slate-200 text-slate-500'}`}>
                            <DollarSign size={24} />
                        </div>
                        <div>
            ...
            {settings?.global_enabled && (<>
                ...
                <div>
                    <h3 className="text-base font-bold text-slate-800 mb-3 flex items-center gap-2">
                        <Zap size={18} className="text-indigo-600" /> Módulos activos
                    </h3>
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        {[
                            ...
                            <div key={m.key} className={`border rounded-lg p-4 flex items-start justify-between transition-all ${settings[m.key] ? 'border-indigo-300 bg-indigo-50' : 'border-slate-200 bg-white'}`}>
                                <div className="flex items-start gap-3">
                                    <div className={`p-2 rounded-lg mt-0.5 ${settings[m.key] ? 'bg-indigo-100 text-indigo-700' : 'bg-slate-100 text-slate-500'}`}>
                                        {m.icon}
                                    </div>
                                    <div>
                    ...
                    <div className="mt-3 p-3 bg-indigo-50 border border-indigo-200 rounded-lg text-xs text-indigo-700">
                ...
                <div>
                    <div className="flex items-center justify-between mb-3">
                        <h3 className="text-base font-bold text-slate-800 flex items-center gap-2">
                            <Tag size={18} className="text-indigo-600" /> Reglas por Categoría
                        </h3>
                        <button onClick={() => { setEditingRule(null); setShowRuleModal(true); }}
                            className="flex items-center gap-1.5 px-4 py-2 bg-indigo-600 text-white rounded-md text-sm font-semibold hover:bg-indigo-700 transition-colors">
                        ...
                        <table className="w-full text-sm">
                            ...
                            <tbody className="divide-y divide-slate-100">
                                {userRates.map(u => {
                                    ...
                                    return (
                                        <tr key={u.user_id} className={`transition-colors ${isEditing ? 'bg-indigo-50' : 'hover:bg-slate-50'}`}>
                                            <td className="p-3">
                                            ...
                                            <td className="p-3 text-center">
                                                {isEditing ? (
                                                    <input type="number" min="0" max="100" step="0.5"
                                                        ...
                                                        className="w-20 p-1.5 border border-indigo-400 rounded-md text-center text-sm font-bold outline-none" />
                                                ) : (
                                                    <span className={`inline-block px-2.5 py-1 rounded-md font-bold text-xs border ${Number(u.commission_vendor_pct) > 0 ? 'bg-indigo-100 text-indigo-800 border-indigo-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
                                            ...
                                            <td className="p-3 text-center">
                                                {isEditing ? (
                                                    ...
                                                    <span className={`inline-block px-2.5 py-1 rounded-md font-bold text-xs border ${Number(u.commission_technician_pct) > 0 ? 'bg-indigo-100 text-indigo-800 border-indigo-200' : 'bg-slate-100 text-slate-500 border-slate-200'}`}>
