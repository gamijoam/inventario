    ...
    return (
        <>
            ...
            <div className="md:hidden space-y-3 overflow-y-auto">
                {orders.length === 0 ? (
                    <div className="bg-white rounded-xl p-8 text-center text-gray-400 italic border border-gray-200">
                        No hay órdenes registradas
                    </div>
                ) : (
                    orders.map(order => {
                        ...
                        return (
                            <div
                                key={order.id}
                                onClick={() => onSelectOrder(order)}
                                className="bg-white rounded-xl p-4 shadow-sm border border-gray-200 active:bg-blue-50 transition-colors"
                            >
                                {/* Header */}
                                ...
                                <div className="grid grid-cols-2 gap-3 mb-3">
                                    <div>
                                        <div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Servicios</div>
                                        <div className="font-bold text-gray-800 text-sm">{meta.total_items || 1} Ítems</div>
                                    </div>
                                    <div>
                                        <div className="text-[10px] text-gray-400 uppercase font-bold mb-1">Estado</div>
                                        {getStatusBadge(order.status)}
                                    </div>
                                </div>
                                ...
                                <div className="flex flex-wrap gap-2">
                                    ...
                                    {order.priority === 'URGENT' && (
                                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-rose-100 text-rose-700 text-xs font-bold border border-rose-200 uppercase">
                                            <AlertTriangle size={12} /> Urgente
                                        </span>
                                    )}
                                    {order.priority === 'HIGH' && (
                                        <span className="inline-flex items-center gap-1 px-2 py-1 rounded-full bg-orange-100 text-orange-700 text-xs font-bold border border-orange-200 uppercase">
