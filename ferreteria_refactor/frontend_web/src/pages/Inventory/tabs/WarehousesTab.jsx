...
const WarehousesTab = () => {
    ...
    return (
        <div className="space-y-4">
                ...
                <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
                    <div className="min-w-0">
                        <h2 className="text-lg font-black text-slate-900 flex items-center gap-2">
                            <Warehouse className="text-indigo-600" size={20} /> Almacenes
                        </h2>
                        <p className="text-xs font-medium text-slate-400">
                            {warehouses.length} registrados - {activeCount} activos - Principal: {mainWarehouse?.name || 'sin definir'}
                        </p>
                    </div>
                    <button
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                {warehouses.map(warehouse => (
                    <div key={warehouse.id} className={clsx(
                        "rounded-lg border p-4 transition-all duration-200 hover:shadow-md relative overflow-hidden group",
                        warehouse.is_main
                            ? 'border-amber-200 bg-amber-50/30'
                            : 'bg-white border-slate-200 shadow-sm'
                    )}>
                        {warehouse.is_main && (
                            <div className="absolute top-0 right-0 bg-amber-400 text-white text-[10px] font-bold px-3 py-1 rounded-bl-lg flex items-center gap-1 shadow-sm uppercase tracking-wider">
                        ...
                        <div className="flex items-start gap-4 mb-6">
                            <div className={clsx(
                                "p-3 rounded-lg flex items-center justify-center shadow-sm",
                                warehouse.is_main ? "bg-amber-100/50 text-amber-600" : "bg-slate-100 text-slate-500"
                            )}>
                                <Warehouse size={28} />
                            </div>
                            <div className="flex-1 min-w-0">
                                <h3 className="text-base font-black text-slate-800 mb-1 truncate">{warehouse.name}</h3>
                ...
                {warehouses.length === 0 && (
                    <div className="col-span-full py-16 text-center bg-slate-50 rounded-lg border-2 border-dashed border-slate-200">
                        ...
                        <p className="text-slate-500">Crea el primer almacen para comenzar a gestionar tu inventario</p>
                    </div>
                )}
            </div>
            ...
            {/* MODAL: CREATE/EDIT */}
            {isModalOpen && (
                <div className="fixed inset-0 bg-indigo-950/45 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
    ...
    return (
        <div className="fixed inset-0 bg-indigo-950/45 backdrop-blur-sm flex items-center justify-center z-50 p-4 animate-in fade-in duration-200">
            <div className="bg-white rounded-lg shadow-xl w-full max-w-3xl flex flex-col max-h-[85vh]">
                        ...
                        <table className="min-w-full divide-y divide-slate-100">
                            <thead className="bg-slate-50 sticky top-0 z-10">
                                <tr>
                                    ...
                                    <th className="px-6 py-3 text-left text-xs font-bold text-slate-500 uppercase tracking-wider">Ubicacion</th>
                                </tr>
                            </thead>
                ...
                <div className="p-4 border-t border-slate-100 bg-slate-50/50 text-right rounded-b-lg">
