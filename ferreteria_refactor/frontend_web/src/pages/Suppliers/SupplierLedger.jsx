...
// Simple local component for currency formatting
        ...
        <div className="p-6">
            ...
            <div className="flex flex-col md:flex-row justify-between items-start md:items-center mb-6 bg-white p-4 rounded-lg shadow print:shadow-none">
                <div>
                    ...
                    >
                        &larr; Volver a Cuentas por Pagar
                    </button>
                    <h1 className="text-2xl font-bold text-gray-800">Historial: {data.supplier.name}</h1>
                    <p className="text-gray-500">ID Proveedor: {data.supplier.id}</p>
                </div>
                <div className="mt-4 md:mt-0 text-right">
                    ...
                    <div className="text-xs text-gray-400 mt-1">Límite: <FormatCurrency amount={data.supplier.limit} currency="USD" /></div>
                </div>
            </div>
                ...
                <table className="min-w-full divide-y divide-gray-200">
                    <thead className="bg-gray-50 print:bg-gray-100">
                        <tr>
                            ...
                            <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Compra (Deuda +)</th>
                            <th className="px-6 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider">Pago (Deuda -)</th>
                            ...
                            data.ledger.map((row, index) => (
                                <tr key={index} className="hover:bg-gray-50">
                                    ...
                                    <td className="px-6 py-4 whitespace-nowrap text-center">
                                        <span className={`px-2 inline-flex text-xs leading-5 font-semibold rounded-full ${row.type === 'COMPRA' ? 'bg-orange-100 text-orange-800' : 'bg-emerald-100 text-emerald-800'
                                            }`}>
                                            {row.type}
                                        </span>
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500">
                                        {row.type === 'COMPRA' ? <FormatCurrency amount={row.debit} currency="USD" /> : '-'}
                                    </td>
                                    <td className="px-6 py-4 whitespace-nowrap text-sm text-right text-gray-500">
                                        {row.type === 'PAGO' ? <FormatCurrency amount={row.credit} currency="USD" /> : '-'}
                                    </td>
